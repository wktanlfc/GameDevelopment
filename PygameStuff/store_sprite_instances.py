...
class store_sprite_instances:
    def __init__(self, surface, cols, rows, width, height, NPC = bool):
    ...
    def sprite_dict(self):
        if self.cols == 3 and self.rows == 4 and self.isNPC is False:
        ...
        elif self.cols == 4 and self.rows == 4 and self.width == 52 and self.isNPC is False: # boxing frame width
            for i in range(self.rows):  # Height
                for j in range(self.cols):  # Width
                    if i <= 1:
                        self.image_store_left.append(self.surface.subsurface((j * self.width, i * self.height), (self.width, self.height)).convert_alpha())
        ...
        ## For NPC Sprites ---------------------------------------
        elif self.cols == 3 and self.rows == 4 and self.isNPC is True:
            for i in range(self.rows):  # Height
                for j in range(self.cols):  # Width
                    if i == 0:
                        self.image_store_down.append(self.surface.subsurface(((j+3) * self.width, i * self.height), (self.width, self.height)).convert_alpha())
                    elif i == 1:
                        self.image_store_left.append(self.surface.subsurface(((j+3) * self.width, i * self.height), (self.width, self.height)).convert_alpha())
                    elif i == 2:
                        self.image_store_right.append(self.surface.subsurface(((j+3) * self.width, i * self.height), (self.width, self.height)).convert_alpha())
                    else:
                        self.image_store_up.append(self.surface.subsurface(((j+3) * self.width, i * self.height), (self.width, self.height)).convert_alpha())
            self.direction_sprites['move_left'] = self.image_store_left
            self.direction_sprites['move_right'] = self.image_store_right
            self.direction_sprites['move_up'] = self.image_store_up
            self.direction_sprites['move_down'] = self.image_store_down
